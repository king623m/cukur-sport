...
client.on('ready', () => {
	...
	client.user.setActivity(`${prefix}X1 | X1`, { type: 'WATCHING' });
});
...
client.on('message', message => {
	if (message.content.toLowerCase() === prefix + 'help') {
		...
		let help = new Discord.MessageEmbed()
			...
			.addField(
				`Protection`,
				`> **${prefix}setlimtban ${prefix}setlimtkick ${prefix}setdelay**`,
				true
			)
			.addField(
				`User`,
				`> **${prefix}set myname ${prefix}set myage ${prefix}set mycountry ${prefix}set mycity ${prefix}set mybio ${prefix}set myinstagram ${prefix}set mytwitter**`,
				true
			)
...
client.on('message', message => {
	...
	if (
		...
		message.content.startsWith('بان')
	) {
		...
		message.author.send(`**> Your Action limit per day: \`\` ${
			banlimt4[message.author.id].banLimit
		}\\${limtnum}
> Action : Banlimt.\`\`**`);
	}
...
client.on('message', message => {
	...
	if (
		...
		message.content.startsWith('حظر')
	) {
		...
		message.author.send(`**> Your Action limit per day: \`\` ${
			kicklimt14[message.author.id].kickLimit
		}\\${limtnum}
> Action : Kicklimt.\`\`**`);
	}
...
client.on('message', message => {
	...
	if (message.content.startsWith(prefix + 'new')) {
		...
		if (
			...
			return message.channel.send(`**Your Already Have Ticket.**`);
		...
		if (k2rba[message.author.id].limt === cnfg.limttick)
			return message.reply(
				`**ERORR: You need to close a ticket from \`${
		...
		message.guild.channels
			...
			.then(async ticket => {
				...
				if (k2rba[message.author.id])
					k2rba[message.author.id].channelname = `ticket-${
				...
				ticket.send(`${Support} ..`).then(m => {
					setTimeout(function() {
						m.edit(`**Please wait for the \`Support\` to come.**`);
					}, 5000);
				});
			});
	}
